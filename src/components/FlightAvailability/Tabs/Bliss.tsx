...
const Bliss = (props: flightAvaliabilityTab) => {
  const {
    router,
    PnrCode,
    showModal,
                            ...
                            <Fragment key={`bliss2-${index}`}>
                              ...
                              {sortedFlights?.map(
                                (dt: flightInfo, listIndex: number) => {
                                  return (
                                    <Fragment key={`bliss3-${listIndex}`}>
                                      {listIndex >= index * 3 &&
                                        listIndex < (index + 1) * 3 && (
                                          <div
                                            className={`p-2 border flex flex-col text-black  justify-center items-center text-center cursor-pointer ${
                                              selectFlight?.display &&
                                              selectFlight?.index ===
                                                listIndex &&
                                              dt?.TotalAmount > 0 &&
